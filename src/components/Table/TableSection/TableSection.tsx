...
const TableSection: FC<IProps> = ({ section, sections, setSections }) => {
  const getNewSections = (tasks: any) => {
    ...
    return sections.map((item: any) =>
      item.sectionId === section.sectionId ? newSection : item
    );
  };
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      newSectionTasks.splice(end, 0, removedTask); //newsectiontasks ...
      ...
      const abort = newSectionTasks.every(
        (task) => task.id !== Number(endTaksId)
      );

      if (abort) return;

      const result = newSectionTasks.map(
        (
          task: any //result ....
        ) =>
          task.id === endTaksId
            ? { ...task, subtasks: [...task.subtasks, removedTask] }
            : task
      );
